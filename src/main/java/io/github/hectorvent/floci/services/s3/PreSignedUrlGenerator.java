...
public class PreSignedUrlGenerator {
    ...
    PreSignedUrlGenerator(String secret, int defaultExpiry, boolean validateSignatures,
                          String defaultRegion, String defaultAccountId) {
        this(secret, defaultExpiry, validateSignatures, defaultRegion, defaultAccountId, Clock.systemUTC());
    }

    PreSignedUrlGenerator(String secret, int defaultExpiry, boolean validateSignatures,
                          String defaultRegion, String defaultAccountId, Clock clock) {
        this.secret = secret;
    ...
    private static List<String> headerValues(String name, URI requestUri,
        ...
        if ("host".equals(name)) {
            for (Map.Entry<String, List<String>> entry : headers.entrySet()) {
                if ("host".equalsIgnoreCase(entry.getKey()) && !entry.getValue().isEmpty()) {
                    return entry.getValue();
                }
            }
            return List.of(canonicalHost(requestUri));
        }
        for (Map.Entry<String, List<String>> entry : headers.entrySet()) {
    ...
    private static List<QueryParameter> parseRawQuery(String rawQuery) {
