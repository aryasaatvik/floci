...
public class DockerResourceIdentity {
    ...
    private String resolve() {
        ...
        Path identityFile = Path.of(config.storage().persistentPath()).resolve(ID_FILE);
        try {
            ...
            String ephemeral = "ephemeral:" + UUID.randomUUID();
            LOG.warnv("Could not persist Docker resource identity at {0}; using {1}: {2}",
                    identityFile, ephemeral, e.getMessage());
            return ephemeral;
        }
    }
}
