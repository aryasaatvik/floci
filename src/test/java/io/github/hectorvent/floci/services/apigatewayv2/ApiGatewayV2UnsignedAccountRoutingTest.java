...
class ApiGatewayV2UnsignedAccountRoutingTest {
    ...
    void unsignedRequestsResolveTheAccountFromApiId() {
        ...
        given()
                .when().get("/execute-api/" + apiB + "/$default/health")
                .then()
                .statusCode(404)
    ...
    private String createApiGraph(String accountId, String functionName) {
        ...
        given()
                ...
                .header("Authorization", authorization)
                .body("""
                        {"stageName":"$default","autoDeploy":true,"tags":{"owner":"%s"}}
                        """.formatted(accountId))
    ...
    private void assertStageOwner(String apiId, String accountId) {
        given()
                .contentType("application/x-amz-json-1.1")
                .header("X-Amz-Target", "AmazonApiGatewayV2.GetTags")
                .header("Authorization", authorization(accountId, "apigatewayv2"))
                .body("""
                        {"ResourceArn":"arn:aws:apigateway:us-east-1::/apis/%s/stages/$default"}
                        """.formatted(apiId))
                .when().post("/")
                .then()
                .statusCode(200)
                .body("Tags.owner", org.hamcrest.Matchers.equalTo(accountId));
    }
