...
class EsmTaggingIntegrationTest {
    ...
    @Test
    @Order(3)
    void createEventSourceMappingPersistsIdentityAndTags() {
        esmUuid = given()
                .contentType("application/json")
                .body("""
                        {
    ...
    @Test
    @Order(5)
    void mismatchedMappingArnReturnsNotFound() {
        given()
                .when()
                .get(TAG_BASE + "/tags/arn:aws:lambda:" + REGION + ":000000000001"
                        + ":event-source-mapping:" + esmUuid)
                .then()
                .statusCode(404)
    ...
    @Test
    @Order(6)
    void cleanupEventSourceMapping() {
        given()
                .when()
